...
public class AutoMail {
	public static void main(String[] args) throws AddressException, FileNotFoundException, IOException {
		...
		mailInfo.setMailServerPort(ReadProperties.getprop("mail", "MailServerPort"));	//mail.properties
		mailInfo.setValidate(true);	//验证
		...
		String cssvalue=ReadHtml.readString("./target/surefire-reports/html/reportng.css");	//将reportng.css的超链接变成普通在html文件里的头
		String changestr="<style type=\"text/css\">h1 {display : inline}"+cssvalue+"</style>";
		mailcontent=mailcontent.replace("<link href=\"reportng.css\" rel=\"stylesheet\" type=\"text/css\" />", changestr);	//替换css超链接
		String logostr="<h1 style=\"color:red ; font-size:50px;font-family: '楷体','楷体_GB2312';\">MAVEN_DEMO</h1><h1>自动化测试报告</h1>";
		mailcontent=mailcontent.replace("<h1>Test Results Report</h1>", logostr);	//替换html标题
